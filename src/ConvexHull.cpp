...
size_t getFarthest(const point2d& a, const point2d& b,
                   const vector<point2d>& v) {
  size_t idxMax = 0;
...
void sort_index(vector<double>& in_v, vector<int>& out_idx) {
  ...
  sort(out_idx.begin(), out_idx.end(),
       [&](int i, int j) { return in_v[i] < in_v[j]; });
}

// r: e_polar[i][0], theta := e_polar[i][1]
// vector<double[2]>
void compute_quickHull_polar(vector<std::array<double, 2U>> e_polar,
                             vector<std::array<double, 2>>& out) {
  unsigned e_N = e_polar.size();
  unsigned N;
  vector<point2d> v;

  for (unsigned int i = 0; i < e_N; i++) {
    double x = e_polar[i][0] * cos(e_polar[i][1]),
           y = e_polar[i][0] * sin(e_polar[i][1]);
    v.push_back(point2d(x, y));
  }
  ...
  for (unsigned int i = 0; i < N; i++) {
    double x = (double)(h[i].x), y = (double)(h[i].y), r = sqrt(x * x + y * y),
           o = atan2(y, x);
    rv[i] = r;
    ov[i] = o;
  }
