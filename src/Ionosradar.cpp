...
// ---------------------------------------------------------------------------------------
void Ionosradar::load_ISR_data_in_time(
    std::vector<std::array<double, 8>>& radar_data, int time_sample) {
  for (H5::Group& beamGroup : beamGroups) {
...
void Ionosradar::compute_fourier_convex_hull(
    vector<std::array<double, 2>>& coords, vector<double>& angles,
    double L_scale) {
  ...
  for (unsigned int i = 0; i < Np - 1; i++) {  // % increased
    ...
    xA0 = xA0 + 0.25 * (b - a) * (xm * (a + b) + 2 * xc) / M_PI;
    yA0 = yA0 + 0.25 * (b - a) * (ym * (a + b) + 2 * yc) / M_PI;
...
// -----------------------------------------------------------------------------------
void Ionosradar::write_interpolated_time_vals_vtk(string data_name,
                                                  unsigned int time,
                                                  double* zi) {
...
// -----------------------------------------------------------------------------------
void Ionosradar::output_radar_data_vtk(string name,
...
void Ionosradar::generateInterpolationGrid(int e_nx, int e_ny, int e_nz) {
  ...
  double avg_theta = acos(avg_dir.z), avg_phi = atan2(avg_dir.y, avg_dir.x);
  ...
  double elmAngVal = 0.0, azmAngVal = 0.0, cx = cos(elmAngVal) * sin(azmAngVal),
         cy = cos(elmAngVal) * cos(azmAngVal), cz = sin(elmAngVal), dot_v0_avr;
  ...
  vector<std::array<double, 2>> convex_hull_data(convex_hull_polar_data.size());
  ...
  for (unsigned int i = 0; i < unit_beam_dir_array.size(); i++) {
    ...
    if (false)
      printf("\t\t % .5f, \t % .5f, % .5f \n", max_R, min_max_range_array[i][0],
             min_max_range_array[i][1]);
  ...
  std::vector<double> top_poly_points(n_poly), top_poly_coeff(n_poly * n_poly);
  ...
  scaled3D_TI_Fourier_CH_Poly_top_grid(nx, ny, nz, Pc_box, n_poly,
                                       top_poly_points, top_poly_coeff, pi, Rb,
                                       RC, LM);
...
double* Ionosradar::shepard_interp_der_2d(int p, unsigned int nd, double* xd,
                                          double* fd, unsigned int ni,
                                          double* xi, bool estimate_derivatives,
                                          double R, double RM, double c_nu)
...
//************************************************************************************
{
  ...
  for (unsigned int i = 0; i < ni; i++) {
    ...
    if (abs(Fi[i]) < 1e-14) {  // in some cases the radar data is missing in the
...
// -----------------------------------------------------------------------------------
// std::array<double,3>
double Ionosradar::top_lead_interpolation(
    double x, double y, std::vector<glm::dvec3>& V, unsigned int nd_e,
    double* xd_e,
  ...
  R = shepard_interp_der_2d(2.0, nd_e, xd_e, fd_e, 1, xit, true, 1000.0, 1000.0,
...
void Ionosradar::scaled3D_TI_Fourier_CH_Poly_top_grid(
    ...
    unsigned int n_poly, std::vector<double>& top_poly_points,
    std::vector<double>& top_poly_coeff, std::vector<std::array<double, 3>>& P,
    double R_rm, double R_RM, double LM) {
  double dx = 1.0 / (nx - 1);
  double dy = 1.0 / (ny - 1);
  double dz = 1.0 / (nz - 1);
    ...
    for (unsigned int j = 0; j < ny; j++) {
      for (unsigned int k = 0; k < nz; k++) {
        point = transfinite_interpolation_Fourier_CH_Poly_top(
            dx * i, dy * j, dz * k, V, n_poly, top_poly_points, top_poly_coeff,
            R_rm, R_RM, LM);
...
void Ionosradar::collect_BeamData(
  ...
  double* azmAngData = read1DData<double>(H5::PredType::NATIVE_DOUBLE, azmAng,
                                          0, numOfazmAng, numOfazmAng);
  double* elmAngData = read1DData<double>(H5::PredType::NATIVE_DOUBLE, elmAng,
                                          0, numOfelmAng, numOfelmAng);
...
// -----------------------------------------------------------------------------------
void Ionosradar::collect_BeamData_Large(
    H5::Group& beamGroup, std::vector<std::array<double, 8>>& data_array,
...
void Ionosradar::produceNANResult(int timestep) {
  ...
  for (unsigned i = 0U; i < ni; i++) {
    ...
    if (verbose)
      printf("i=%d:\tISR(%5.3f,\t%5.3f,\t%.3e,\t%.3e,\t%.3e)\n", i,
             data_interp[i][0], data_interp[i][1], data_interp[i][2],
             data_interp[i][3], data_interp[i][4]);
  }
