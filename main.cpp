...
#include "util/ConvexHull.hpp"

#include "util/lac.hpp"
#include "util/reader_hdf5.hpp"
...
class Ionosradar {
 private:
  string ROOT_GROUP, PAR_1D, PAR_2D,
 ...
 public:
  ...
  std::vector<Group> beamGroups;
  Group beam_closest_z_axis;
  ...
  fstream file;
  string data_name, data_path, output_folder, vtk_filename;

  void print_metadata() { ISR_meta.print(); }
  ...
  void load_radar_data_info(const std::string& directory,
    ...
    for (Group& beamGroup : beamGroups) {
  ...
  */
  ...
  double min_angle_sq_fit_data(vector<std::array<double, 2>>& X) {
  ...
  void compute_fourier_convex_hull(vector<std::array<double, 2>>& coords,
  ...
  // -----------------------------------------------------------------------------------
  void write_interpolated_time_vals_vtk(string data_name, unsigned int time,
                                        double* zi) {
 ...
 private:
  ...
  void collect_BeamData(
      Group& beamGroup, std::vector<std::array<double, 3>>& points_array,
      std::vector<std::array<double, 3>>& points_max_array,
      std::vector<std::array<double, 3>>& points_max_array_polar,
      std::vector<glm::dvec3>& unit_beam_dir_array,
      std::vector<std::array<double, 2>>& min_max_range_array,
      std::array<double, 6>& beam_bounding_box) {
    Group params1d = beamGroup.openGroup(PAR_1D);
    Group params2d = beamGroup.openGroup(PAR_2D);
    DataSet range = beamGroup.openDataSet(RANGE_DATA);
    DataSet time = beamGroup.openDataSet(TIME_DATA);

    DataSet nel_beamDataSet =
        params2d.openDataSet(BEAM_DATA_NEL);  // BEAM_DATA_UNEL

    DataSet azmAng = params1d.openDataSet(ANG_AZM_DATA);
    DataSet elmAng = params1d.openDataSet(ANG_ELM_DATA);
    ...
    double* azmAngData = read1DData<double>(PredType::NATIVE_DOUBLE, azmAng, 0,
                                            numOfazmAng, numOfazmAng);
    double* elmAngData = read1DData<double>(PredType::NATIVE_DOUBLE, elmAng, 0,
                                            numOfelmAng, numOfelmAng);
    double* rangeData = read1DData<double>(PredType::NATIVE_DOUBLE, range, 0,
                                           numOfRange, numOfRange);

    int* timeData =
        read1DData<int>(PredType::NATIVE_INT, time, 0, numOfTime, numOfTime);

    // double lastLen = rangeData[numOfRange - 1];

    time_measurements = numOfDimSamples.rows;

    DataSet beamid_set = params1d.openDataSet("beamid");
    hsize_t numOfBeamsData = getDimensions(beamid_set).cols;
    int* beamid = read1DData<int>(PredType::NATIVE_INT, beamid_set, 0,
  ...
  // -----------------------------------------------------------------------------------
  void collect_BeamData_Large(Group& beamGroup,
    ...
    DataSet pop_beamDataSet, vo_beamDataSet, Ti_beamDataSet, Te_beamDataSet;
    Array2D<double> nel_data, pop_data, vo_data, ti_data, te_data;

    nel_data = read2DData<double>(PredType::NATIVE_DOUBLE, nel_beamDataSet,
    ...
    if (num_scalar_data > 4) {
      ...
      pop_data = read2DData<double>(PredType::NATIVE_DOUBLE, pop_beamDataSet,
                                    {0, 0}, numOfDimSamples, numOfDimSamples);
      vo_data = read2DData<double>(PredType::NATIVE_DOUBLE, vo_beamDataSet,
                                   {0, 0}, numOfDimSamples, numOfDimSamples);
      ti_data = read2DData<double>(PredType::NATIVE_DOUBLE, Ti_beamDataSet,
                                   {0, 0}, numOfDimSamples, numOfDimSamples);
      te_data = read2DData<double>(PredType::NATIVE_DOUBLE, Te_beamDataSet,
                                   {0, 0}, numOfDimSamples, numOfDimSamples);
    }
...
int main(int argc, char** argv) {
  ...
  unsigned int nx = 32, ny = 32, nz = 32,
